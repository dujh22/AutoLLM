...
def get_image_blending(image, face, face_box, mask_array, crop_box):
    ...
    # 检查 mask_array 的形状
    if len(mask_array.shape) == 2:  # 单通道图像
        mask_image = mask_array  # 如果已经是灰度图，则直接使用
    elif len(mask_array.shape) == 3 and mask_array.shape[2] == 3:  # 三通道图像
        mask_image = cv2.cvtColor(mask_array, cv2.COLOR_BGR2GRAY)
    else:
        raise ValueError("Invalid shape for mask_array: {}".format(mask_array.shape))
