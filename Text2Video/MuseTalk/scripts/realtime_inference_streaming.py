...
if __name__ == "__main__":
    ...
    avatar_id = list(inference_config.keys())[0]
    ...
    while True:
        audio_files = glob.glob(os.path.join(inference_config[avatar_id]["audio_dir"], "*.wav"))
        for audio_file in audio_files:
            print("Inferring using:", audio_file)
            audio_num = os.path.splitext(os.path.basename(audio_file))[0]
            avatar.inference(audio_file, audio_num, args.fps, args.skip_save_images)
            os.rename(audio_file, audio_file + '.finished')
        time.sleep(1)
