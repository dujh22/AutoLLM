...
def generate_video():
    ...
    # 使用FFmpeg进行实时编码，输出为MPEG-TS格式
    process = (
        ffmpeg
        .input('pipe:', format='rawvideo', pix_fmt='rgb24', s='{}x{}'.format(WIDTH, HEIGHT), framerate=FPS)
        .output('pipe:', format='mpegts', vcodec='libx264', pix_fmt='yuv420p', preset='ultrafast',
                tune='zerolatency', fflags='+flush_packets', max_delay=0)
        .global_args('-loglevel', 'error')  # 减少日志输出
        .run_async(pipe_stdin=True, pipe_stdout=True, pipe_stderr=True)
    )

    # 创建一个独立的生成器函数，从FFmpeg的stdout读取数据
    def read_ffmpeg():
        try:
            while True:
            ...
            print("Error reading ffmpeg stdout:", e)
        finally:
            process.stdout.close()

    ffmpeg_output = read_ffmpeg()
...
# 定义Flask路由，提供视频流（使用MPEG-TS格式）
@app.route('/video_feed')
def video_feed():
    return Response(stream_with_context(generate_video()), mimetype='video/mp2t')
...
if __name__ == "__main__":
    ...
    for avatar_id in inference_config:
        ...
        audio_dir = inference_config[avatar_id].get("audio_dir", "")  # 新增，读取audio_dir字段
        if not audio_dir:
            print(f"avatar_id {avatar_id} does not have 'audio_dir' specified.")
            continue
        avatar = Avatar(
            avatar_id=avatar_id,
            video_path=video_path,
